		...
		private void CommitWriteTransaction()
		{
...
#if !KeePassUAP
				// May throw with Mono.
				// File.GetAccessControl(string, AccessControlSections) was removed
				// in .NET Core; use the FileInfo extension method instead.
				FileSecurity sec = new FileInfo(m_iocBase.Path).GetAccessControl(acs);
			...
			try
			{
				...
				if((pbSec != null) && (pbSec.Length != 0))
				{
					...
					// File.SetAccessControl(string, FileSecurity) was removed in .NET Core;
				// use the FileInfo extension method instead.
				new FileInfo(m_iocBase.Path).SetAccessControl(sec);
				}
#endif
			}
