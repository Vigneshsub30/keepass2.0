...
namespace KeePassLib.Security
{
	...
	public sealed class ProtectedBinary : IEquatable<ProtectedBinary>
	{
		...
		private static bool ProtectedMemorySupported
		{
			get
			{
#if KeePassLibSD || !NETFRAMEWORK
				// System.Security.Cryptography.ProtectedMemory is not available
				// outside of .NET Framework. On .NET 5+/Core, the ChaCha20
				// fallback path is used instead (see Encrypt/Decrypt below).
				return false;
#else
